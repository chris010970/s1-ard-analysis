...
def populateTable( plist ):
    ...
    if plist[ 'orbit' ] == 'ASCENDING' or plist[ 'orbit' ] == 'DESCENDING':   
        ...
        query = "CREATE TABLE IF NOT EXISTS error.%s_%s_%s AS ( " \
                    "WITH pts AS ( " \
                        "SELECT a.fdate, ST_AsText(a.geom) geom, a.vv gamma_vv, a.vh gamma_vh, b.vv snap_vv, b.vh snap_vh, a.vv-b.vv vv_error, a.vh-b.vh vh_error, c.filename, c.orbitdirection, c.relativeorbitnumber FROM result_gamma.%s_%s a " \
        ...
        param_list = ( AsIs( plist[ 'product' ] ), AsIs( plist[ 'landcover' ] ), AsIs( plist[ 'orbit' ].lower() ),
        ...
        query = "CREATE TABLE IF NOT EXISTS error.%s_%s AS ( " \
                    "WITH pts AS (  " \
                        "SELECT a.fdate, ST_AsText(a.geom) geom, a.vv gamma_vv, a.vh gamma_vh, b.vv snap_vv, b.vh snap_vh, a.vv-b.vv vv_error, a.vh-b.vh vh_error FROM result_gamma.%s_%s a " \
                            "INNER JOIN result_snap.%s_%s b ON a.fdate = b.fdate AND ST_Equals( a.geom, b.geom ) ) " \
                                "SELECT geom, avg(gamma_vv) gamma_vv_mean, stddev(gamma_vv) gamma_vv_stddev, avg( gamma_vh ) gamma_vh_mean, stddev( gamma_vh) gamma_vh_stddev, avg( snap_vv ) snap_vv_mean, stddev( snap_vv ) snap_vv_stddev, avg( snap_vh ) snap_vh_mean, stddev( snap_vh ) snap_vh_stddev, avg(vv_error) vv_error_mean, stddev( vv_error ) vv_error_stddev, avg( vh_error ) vh_error_mean, stddev( vh_error) vh_error_stddev  " \
    ...
    try:
        ...
        cur.execute( "CREATE SCHEMA IF NOT EXISTS error;"  )
        conn.commit()
